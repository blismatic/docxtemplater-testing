...
const content = fs.readFileSync(
  path.resolve(__dirname, "../templates/list-punctuation-version-c.docx"),
  "binary",
);
    ...
    evaluateIdentifier(tag, scope, scopeList, context) {
      if (tag === "$punc") {
        ...
        return (style: string) => {
          ...
          if (style === "b") {
            if (totalLength === 2 && index === 0) return " and ";
            if (index === totalLength - 2) return ", and ";
            if (index < totalLength - 2) return ", ";
            return "";
          }
          if (style === "c") {
            if (totalLength === 2 && index === 0) return " and ";
...
fs.writeFileSync(
  path.resolve(__dirname, "../outputs/output-punctuation-c.docx"),
  buf,
);
